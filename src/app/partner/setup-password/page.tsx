...
function SetupPasswordContent() {
  ...
  const validateToken = async () => {
    try {
      const response = await fetch(`/api/partner/setup-password?token=${token}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/partner/setup-password", {
        method: "POST",
        ...
        >
          <Card className="border-[#e5e0d5] shadow-2xl">
            <CardContent className="pt-12 pb-8 text-center">
              ...
              <h2 className="text-2xl font-bold text-[#0a192f] mb-2" style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 600 }}>
                You're All Set!
              </h2>
              <p className="text-[#5a7a9a] mb-8">
                Your password has been created successfully. You can now log in to your Partner Dashboard.
              </p>
              <Button
        ...
        >
          <Card className="border-[#e5e0d5] shadow-2xl">
            <CardContent className="pt-12 pb-8 text-center">
              ...
              <p className="text-[#5a7a9a] mb-8">
                {error || "This password setup link is invalid or has expired. Please contact your referral manager for a new invitation."}
              </p>
              <Button
                onClick={() => router.push("/partner/login")}
                variant="outline"
      ...
      >
        <div className="text-center mb-8">
          <Link href="/" className="inline-flex items-center gap-3 mb-6">
            ...
            <span className="text-2xl font-bold text-[#0a192f]" style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 600 }}>
        ...
        <Card className="border-[#e5e0d5] shadow-2xl">
          <CardHeader className="text-center pb-2">
            ...
            <CardTitle className="text-2xl text-[#0a192f]" style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 600 }}>
              Set Up Your Password
            </CardTitle>
            <CardDescription className="text-[#5a7a9a]">
              Create your partner password to get started
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="pt-6">
            ...
            <p className="text-center text-xs text-[#5a7a9a] mt-6">
              ...
              <Link href="/partner/login" className="text-[#c9a227] hover:underline">
                Sign in
              </Link>
            </p>
          </CardContent>
...
export default function PartnerSetupPasswordPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-[#faf8f3] flex items-center justify-center">
        <Loader2 className="w-8 h-8 animate-spin text-[#1e3a5f]" />
