...
export default function AgencyDashboard() {
  ...
  // Knowledge Hub state
  const [khResources, setKhResources] = useState<{ id: string; type: string; title: string; description: string; file_url: string; created_at: string }[]>([])
  ...
  const [newResource, setNewResource] = useState({ type: "course_catalog", title: "", description: "", file_url: "" })
  ...
  const handleAddResource = async () => {
    if (!newResource.title || !newResource.type) { toast.error("Please fill in the required fields"); return }
    setIsSubmittingResource(true)
    try {
      const response = await fetch("/api/agency/knowledge-hub", {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(newResource)
      })
      if (response.ok) {
        ...
        setUploadedFileName("")
        fetchKhResources()
      } else { const error = await response.json(); toast.error(error.error || "Failed to add resource") }
    } catch { toast.error("An error occurred") }
    finally { setIsSubmittingResource(false) }
  }

  const handleDeleteResource = async (id: string) => {
    if (!confirm("Are you sure you want to delete this resource?")) return
    try {
      const response = await fetch(`/api/agency/knowledge-hub?id=${id}`, { method: "DELETE" })
      if (response.ok) { toast.success("Resource deleted"); fetchKhResources() }
      else { toast.error("Failed to delete resource") }
    } catch { toast.error("An error occurred") }
  }

  const filteredKhResources = khResources.filter(r =>
    r.title.toLowerCase().includes(khSearchTerm.toLowerCase()) ||
    r.type.toLowerCase().includes(khSearchTerm.toLowerCase())
  )
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-8 p-8 bg-white rounded-3xl border border-[#e5e0d5] shadow-sm">
            <div>
              <h1 className="text-4xl font-black tracking-tight mb-2" style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 600, color: primaryColor }}>
                Admin Dashboard
              </h1>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="!flex !flex-row gap-6">
              ...
              <TabsList className="!flex !flex-col !w-56 !h-auto items-stretch gap-1 bg-white border border-[#e5e0d5] p-2 rounded-2xl shadow-sm shrink-0 sticky top-24 self-start">
                  ...
                  <TabsTrigger value="students" className="rounded-xl px-4 py-2.5 font-bold text-sm !flex-none justify-start data-[state=active]:bg-[#0a192f] data-[state=active]:text-white">
                  ...
                  <TabsTrigger value="analytics" className="rounded-xl px-4 py-2.5 font-bold text-sm !flex-none justify-start data-[state=active]:bg-[#0a192f] data-[state=active]:text-white">
                    <BarChart3 className="w-4 h-4 mr-2" />
                    Analytics
                  </TabsTrigger>
                  <TabsTrigger value="at-risk" className="rounded-xl px-4 py-2.5 font-bold text-sm !flex-none justify-start data-[state=active]:bg-[#0a192f] data-[state=active]:text-white">
                    <AlertTriangle className="w-4 h-4 mr-2" />
                    At-Risk
                  </TabsTrigger>
                  <TabsTrigger value="manage-admins" className="rounded-xl px-4 py-2.5 font-bold text-sm !flex-none justify-start data-[state=active]:bg-[#0a192f] data-[state=active]:text-white">
                  ...
                  <TabsTrigger value="audit-logs" className="rounded-xl px-4 py-2.5 font-bold text-sm !flex-none justify-start data-[state=active]:bg-[#0a192f] data-[state=active]:text-white">
                    <CheckCircle2 className="w-4 h-4 mr-2" />
                    Audit Logs
                  </TabsTrigger>
                  <TabsTrigger value="knowledge-hub" className="rounded-xl px-4 py-2.5 font-bold text-sm !flex-none justify-start data-[state=active]:bg-[#0a192f] data-[state=active]:text-white">
                    <BookOpen className="w-4 h-4 mr-2" />
                    Knowledge Hub
                  </TabsTrigger>
                  <div className="border-t border-[#e5e0d5] my-1" />
                  <TabsTrigger value="settings" className="rounded-xl px-4 py-2.5 font-bold text-sm !flex-none justify-start data-[state=active]:bg-[#0a192f] data-[state=active]:text-white">
            ...
            <TabsContent value="knowledge-hub" className="space-y-6">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  <h3 className="text-2xl font-black text-[#0a192f]">Knowledge Hub</h3>
                  <p className="text-[#5a7a9a]">Upload school course catalogs, clubs, and extracurricular resources</p>
                </div>
                <Dialog open={showAddResourceDialog} onOpenChange={setShowAddResourceDialog}>
                  <Button onClick={() => setShowAddResourceDialog(true)} className="h-12 px-6 rounded-2xl font-black" style={{ backgroundColor: primaryColor }}>
                  ...
                  <DialogContent className="sm:max-w-md rounded-[32px] p-8">
                    <DialogHeader>
                      ...
                      <DialogDescription>Upload a new resource to your school&apos;s hub</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-6 py-4">
                      ...
                      <div className="space-y-2">
                        <Label>Title</Label>
                        <Input placeholder="e.g., Grade 11 Course Guide 2026" value={newResource.title} onChange={(e) => setNewResource({...newResource, title: e.target.value})} className="h-12 rounded-xl" />
                      </div>
                      <div className="space-y-2">
                        <Label>Description (Optional)</Label>
                        <Textarea placeholder="Describe the resource..." value={newResource.description} onChange={(e) => setNewResource({...newResource, description: e.target.value})} className="rounded-xl" rows={3} />
                      </div>
                      <div className="space-y-2">
                        <Label>Upload File or Enter URL</Label>
                        <input ref={fileInputRef} type="file" className="hidden" accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.png,.jpg,.jpeg,.webp" onChange={handleFileUpload} />
                        <div className="border-2 border-dashed border-[#e5e0d5] rounded-xl p-4 text-center cursor-pointer hover:border-[#c9a227] hover:bg-amber-50/30 transition-all" onClick={() => fileInputRef.current?.click()}>
                          {uploadingFile ? (
                            <div className="flex items-center justify-center gap-2 py-2"><Loader2 className="w-5 h-5 animate-spin text-[#5a7a9a]" /><span className="text-sm text-[#5a7a9a]">Uploading...</span></div>
                          ) : uploadedFileName ? (
                            <div className="flex items-center justify-center gap-2 py-2"><FileText className="w-5 h-5 text-green-600" /><span className="text-sm font-bold text-green-700">{uploadedFileName}</span></div>
                          ) : (
                            <div className="py-2"><Upload className="w-6 h-6 mx-auto mb-1 text-[#5a7a9a]" /><p className="text-sm font-bold text-[#5a7a9a]">Click to upload a file</p><p className="text-xs text-[#5a7a9a]/60">PDF, Word, Excel, CSV, or images (max 10MB)</p></div>
                          )}
                        </div>
                        <div className="flex items-center gap-3"><div className="flex-1 border-t border-[#e5e0d5]" /><span className="text-xs font-bold text-[#5a7a9a]">OR</span><div className="flex-1 border-t border-[#e5e0d5]" /></div>
                        <Input placeholder="Paste a URL..." value={uploadedFileName ? '' : newResource.file_url} onChange={(e) => { setNewResource({...newResource, file_url: e.target.value}); setUploadedFileName("") }} className="h-12 rounded-xl" disabled={!!uploadedFileName} />
                      </div>
                      <Button onClick={handleAddResource} disabled={isSubmittingResource || !newResource.title} className="w-full h-14 rounded-2xl font-black text-lg shadow-lg" style={{ backgroundColor: primaryColor, color: 'white' }}>
                        {isSubmittingResource ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Rocket className="w-4 h-4 mr-2" />}
                ...
                <CardHeader className="p-8">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                      ...
                      <p className="text-sm text-[#5a7a9a]">Manage your uploaded documents and lists</p>
                    </div>
                    <div className="relative w-full md:w-80">
                      <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#5a7a9a]" />
                      <Input placeholder="Search resources..." value={khSearchTerm} onChange={(e) => setKhSearchTerm(e.target.value)} className="pl-11 h-12 rounded-2xl border-[#e5e0d5] bg-[#faf8f3]" />
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="p-0">
                  <Table>
                    ...
                    <TableBody>
                      {filteredKhResources.length === 0 ? (
                        ...
                        filteredKhResources.map((resource) => (
                          <TableRow key={resource.id} className="border-[#e5e0d5] hover:bg-[#faf8f3]/50 transition-colors">
                            ...
                            <TableCell>
                              <span className="text-sm text-[#5a7a9a] font-medium">{new Date(resource.created_at).toLocaleDateString()}</span>
                            </TableCell>
                            <TableCell className="px-8 text-right">
                              <div className="flex justify-end gap-2">
                                {resource.file_url && (
                                  <Button size="sm" variant="ghost" className="rounded-xl h-11 w-11 p-0 text-[#1e3a5f] hover:bg-blue-50" onClick={() => window.open(resource.file_url, '_blank')}>
                                    <ExternalLink className="w-5 h-5" />
                                  </Button>
                                )}
                                <Button size="sm" variant="ghost" className="rounded-xl h-11 w-11 p-0 text-red-500 hover:bg-red-50" onClick={() => handleDeleteResource(resource.id)}>
            ...
            <TabsContent value="settings">
              <Suspense fallback={<div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-[#0a192f]" /></div>}>
                <AgencySettingsContent embedded />
              </Suspense>
            </TabsContent>
